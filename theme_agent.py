...
#
# Usage:
#   python theme_agent.py                  # reads GEMINI_API_KEY from .env
#   python theme_agent.py --mock           # ETF-momentum proxy, no API key needed
...
Given recent market context for each theme, assign a cross-sectional attractiveness score.
...
- 0.0 = neutral.
- Use ETF momentum as one input, but also consider macro/industry logic.
...
def score_themes_gemini(
    ...
    response = client.models.generate_content(
        model=model,
        contents=build_user_prompt(themes, context, as_of),
...
def score_themes(
    ...
    *,
    ...
    if use_llm:
        scores = score_themes_gemini(themes, context, as_of_str)
    else:
        scores = score_themes_mock(themes, context)

    if save:
        save_theme_scores(as_of_str, scores, context, source="gemini" if use_llm else "mock")
...
def main() -> None:
    ...
    source = "gemini" if use_llm else "mock"

    if use_llm:
        scores = score_themes_gemini(themes, context, as_of)
        if not args.no_save:
            save_theme_scores(as_of, scores, context, source=source)
    else:
        scores = score_themes_mock(themes, context)
        if not args.no_save:
            save_theme_scores(as_of, scores, context, source=source)

    print_scores(scores, as_of, source)
    if not args.no_save:
